...
const handleCart = async (req: any, res: any) => {
    try {
        ...
        const existingCartItemQuery = db.collection('cart')
                                        .where('email', '==', email)
                                        .where('roomId', '==', roomId);
...
const getCartItems = async (req: any, res: any) => {
    try {
        ...
        res.status(200).json({ message: "Cart items fetched successfully", cartItems: cartItemsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) });
    } catch (error) {
