...
const handleBooking = async (req: any, res: any) => {
    ...
    try {
        ...
        res.status(error.status || 500).json({ message: error.message || "Internal server error during booking." });
    }
};
...
const updateBooking = async (req: any, res: any) => {
    const transaction = db.runTransaction(async t => {
        try {
            ...
            const existingRoomIds: string[] = Array.isArray(existingBookingData?.roomIds)
                ...
                : (existingBookingData?.roomIds ? [existingBookingData.roomIds] : []);


            let newStartDateTime: Date | undefined = existingBookingData?.startDate ? new Date(existingBookingData.startDate) : undefined;
            let newEndDateTime: Date | undefined = existingBookingData?.endDate ? new Date(existingBookingData.endDate) : undefined;
    ...
    try {
        ...
        res.status(error.status || 500).json({ message: error.message || "Internal server error during booking update." });
    }
};
